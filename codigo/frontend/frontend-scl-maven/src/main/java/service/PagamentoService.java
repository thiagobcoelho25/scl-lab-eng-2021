...
private final String url = "http://localhost:8080/scl/pagamentos";
	...
	public List<Pagamento> listAll(){
		try {
            ...
            TypeReference<List<Pagamento>> mapType = new TypeReference<List<Pagamento>>() {
            };
            List<Pagamento> lista = mapper.readValue(json, mapType);
            return lista;
        } catch (IOException ex) {
            Logger.getLogger(PagamentoService.class.getName()).log(Level.SEVERE, null, ex);
        }
		return null;
	}
