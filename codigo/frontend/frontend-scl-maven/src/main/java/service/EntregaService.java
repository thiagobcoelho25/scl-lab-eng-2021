...
public class EntregaService {
	private final String url = "http://localhost:8080/scl/entregas";
	...
	public List<Entrega> listAll() {
		try {
			...
			TypeReference<List<Entrega>> mapType = new TypeReference<List<Entrega>>() {
			};
			List<Entrega> lista = mapper.readValue(json, mapType);
			return lista;
		} catch (IOException ex) {
			Logger.getLogger(EntregaService.class.getName()).log(Level.SEVERE, null, ex);
		}
		return null;
	}
	...
	public String insert(Entrega entrega) {
		try {
			WebTarget target = client.target(url);
			ObjectMapper mapper = new ObjectMapper();
			String json = mapper.writeValueAsString(entrega);
	...
	public String construçãoStandardError(StandardError standardError) {
		return standardError.getMessage() == null
