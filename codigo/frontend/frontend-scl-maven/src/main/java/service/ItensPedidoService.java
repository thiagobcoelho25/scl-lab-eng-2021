...
public class ItensPedidoService {
	private final String url = "http://localhost:8080/scl/itenspedido";
	...
	public List<ItensPedido> listAll(){
		try {
            ...
            TypeReference<List<ItensPedido>> mapType = new TypeReference<List<ItensPedido>>() {
            };
            List<ItensPedido> lista = mapper.readValue(json, mapType);
            return lista;
        } catch (IOException ex) {
            Logger.getLogger(ItensPedidoService.class.getName()).log(Level.SEVERE, null, ex);
        }
		return null;
	}

	public String insert(ItensPedido itensPedido) {
		try {
            WebTarget target = client.target(url);
            ObjectMapper mapper = new ObjectMapper();
            String json = mapper.writeValueAsString(itensPedido);
	...
	public String update(ItensPedido itensPedido) {
        try {
            WebTarget target = client.target(url);
            ObjectMapper mapper = new ObjectMapper();
            String json = mapper.writeValueAsString(itensPedido);
